...
async function test() {
  ...
  const res = await fetch(`https://kernel-core.yowyob.com/api/organizations`, {
    headers: {
  ...
  const orgs = data.data?.content || data.data || [];
  
  console.log("Organizations:");
  orgs.forEach(o => console.log(`${o.id} - ${o.name} - tenantId: ${o.tenantId}`));
}
